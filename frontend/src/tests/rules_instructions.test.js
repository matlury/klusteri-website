...
describe('Instructions component opens', () => {
  ...
  it('Shows the safety rules', () => {
    const { getByText } = render(<Rules_and_Instructions />)
    const safetyButton = getByText('Matlu-klusterin turvallisen tilan periaatteet')
    fireEvent.click(safetyButton)

    expect(getByText('Kunnioita toisia ja heidän omaa tilaa sekä koskemattomuutta')).toBeInTheDocument()
  })
})